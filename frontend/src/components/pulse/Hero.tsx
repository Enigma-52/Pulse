  ...
  <section className="relative pt-32 pb-24 lg:pt-40 lg:pb-32 overflow-hidden">
    ...
    <div className="relative max-w-[1400px] mx-auto px-6 lg:px-10">
      ...
      <div className="flex items-center gap-3 font-mono text-[11px] uppercase tracking-[0.2em] text-muted-foreground mb-10 animate-rise">
        ...
        <span>Open-source observability · Built for engineers</span>
      </div>
      ...
      <h1 className="font-display text-[14vw] sm:text-[10vw] lg:text-[8.5rem] xl:text-[10rem] leading-[0.95] tracking-tighter text-balance animate-rise">
        From symptom to
        <br />
        <span className="italic text-primary">root cause</span>,
        <br />
        in one breath.
      </h1>
      ...
      <div className="mt-12 grid lg:grid-cols-12 gap-8 items-end animate-rise" style={{ animationDelay: '0.15s' }}>
        <p className="lg:col-span-6 text-lg lg:text-xl text-muted-foreground max-w-xl text-pretty">
          Pulse unifies traces, logs, and metrics into a single operational workflow.
          Self-hosted, ClickHouse-backed, and shaped by the way engineers actually debug production.
        </p>
        <div className="lg:col-span-6 flex flex-col sm:flex-row lg:justify-end gap-3">
          <a href="#cta" className="inline-flex items-center justify-center gap-2 bg-primary text-primary-foreground px-6 py-3.5 rounded-sm font-medium hover:bg-primary-glow transition-colors">
      ...
      <div className="mt-20 max-w-3xl mx-auto animate-rise" style={{ animationDelay: '0.3s' }}>
        <div className="rounded-md border border-border bg-surface-1/80 backdrop-blur shadow-2xl overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border bg-surface-2/60">
            ...
            <span className="h-2.5 w-2.5 rounded-full bg-muted" />
            <span className="h-2.5 w-2.5 rounded-full bg-muted" />
            <span className="ml-3 font-mono text-[11px] text-muted-foreground">~/pulse</span>
          </div>
          <pre className="p-5 font-mono text-sm leading-relaxed text-foreground/90">
<span className="text-muted-foreground">$</span> curl -fsSL pulse.dev/install | sh{"\n"}
<span className="text-signal-metric">✓</span> <span className="text-muted-foreground">ingestion · processing · clickhouse · api · ui</span>{"\n"}
<span className="text-signal-metric">✓</span> <span className="text-muted-foreground">stack online in 47s</span>{"\n"}
<span className="text-muted-foreground">$</span> pulse trace search --status=5xx --p95{">"} 800ms<span className="inline-block w-2 h-4 bg-primary align-middle ml-1 animate-blink" />
          </pre>
        </div>
      </div>
    </div>
